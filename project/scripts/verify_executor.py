"""One-off verification of code_executor.py against an existing samples file."""
...
def main() -> None:
    ...
    for i, sample in enumerate(samples, 1):
        ...
        result = run_executor(task_id=task_id, completion=completion, dataset="humaneval")
    ...
    if all_not_passed:
        show = all_not_passed[:10]
        print(f"\n{'=' * 60}")
        print(f"FIRST {len(show)} FAILED/TIMED-OUT TASKS (detail)")
        print("=" * 60)
        for r in show:
            print(f"\n  task_id      : {r['task_id']}")
